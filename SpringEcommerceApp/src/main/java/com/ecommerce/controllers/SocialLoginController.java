...
public class SocialLoginController {
    ...
    @PostMapping("/google")
    @CrossOrigin(origins = {"https://localhost:3000"}, allowCredentials = "true", allowedHeaders = "*", exposedHeaders = {"*"})
    public ResponseEntity<?> googleLogin(@RequestBody GoogleAuthRequest request) {
        try {
            System.out.println("=== GOOGLE LOGIN DEBUG ===");
            System.out.println("Request headers: " + Thread.currentThread().toString());
            System.out.println("Request credential length: " + (request.credential != null ? request.credential.length() : "null"));
            ...
            System.out.println("User info - Email: " + email + ", Name: " + name + ", Picture: " + (picture != null) + ", GoogleID: " + googleId);
            ...
            if (existingUser == null) {
                ...
                try {
                    ...
                    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                            .body(new SocialAuthResponse(false, "Lỗi khi tạo tài khoản mới: " + e.getMessage(), null, null));
                }
            } else {
            ...
            try {
                ...
                return ResponseEntity.ok(new SocialAuthResponse(true, "Đăng nhập Google thành công", token, existingUser));
            } catch (Exception e) {
    ...
    public ResponseEntity<?> facebookLogin(@RequestBody FacebookAuthRequest request) {
        try {
            ...
            return ResponseEntity.ok(new SocialAuthResponse(true, "Đăng nhập Facebook thành công", token, existingUser));
    ...
    private Map<String, Object> verifyGoogleToken(String idToken) {
        try {
            ...
            // Lấy client ID từ application.properties hoặc sử dụng danh sách các client ID được chấp nhận
    ...
    private Map<String, Object> verifyFacebookToken(String accessToken, String userId) {
        try {
            ...
            String graphApiUrl = "https://graph.facebook.com/v16.0/me?fields=id,name,email,picture&access_token=" + accessToken;
    ...
    private User createGoogleUser(String email, String name, String picture, String googleId) {
        try {
            ...
            if (userWithUsername != null) {
                System.out.println("WARNING: Username conflict - a user with username=" + email + " exists but with different email");
