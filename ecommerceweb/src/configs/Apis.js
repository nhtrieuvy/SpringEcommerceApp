...
export const endpoint = {
    ...
    GET_PRODUCTS_BY_STORE: (storeId, page = 0, size = 10) => `/api/products?storeId=${storeId}&page=${page}&size=${size}`,
    GET_PRODUCT_BY_ID: (id) => `/api/products/${id}`,
    CREATE_PRODUCT: `/api/products`,
    UPDATE_PRODUCT: (id) => `/api/products/${id}`,
    DELETE_PRODUCT: (id) => `/api/products/${id}`,    SEARCH_PRODUCTS: `/api/products/search`,    // Endpoints cho quản lý sản phẩm của seller
    ...
    UPLOAD_PRODUCT_IMAGE: (id) => `/api/seller/products/${id}/upload-image`,    TOGGLE_PRODUCT_STATUS: (id) => `/api/seller/products/${id}/status`,
    ...
    instance.interceptors.request.use(
        config => {
            ...
            console.log('DEBUG - Request Config:', {
                ...
                data: config.data instanceof FormData ? 'FormData (không hiển thị)' : JSON.stringify(config.data)
            });
            return config;
        },
        error => {
